...
public class MetroGraph2 {
	...
	//returns the end platform for this line farthest from central Boston
	private Platform endOfLine(String trainLine){

		double distance = 0;
		Platform farthest = null;

		for(int i = 1; i < G.V(); i++){
			Platform x = platforms[i];
			//get platform in this line
			if(trainLine.equals(x.getTrainLine())){
				//get platforms on ends
				int count = 0;
				for (int j : G.adj(i)) {
					if (platforms[j].getTrainLine().equals(trainLine)){
					count ++;
					}
				}

				if (count == 1){
					Platform end_plat = platforms[i];
					double curDist = end_plat.distanceFromBoston();
					if (curDist > distance) {
						distance = curDist;
						farthest = end_plat;
					}
	...
	private Set<String> findALLTrainLines(){

		Set<String> trainlines = new TreeSet<String>();

		for(int i = 1; i < G.V(); i++){
			Platform x = platforms[i];
			String line = x.getTrainLine();
			trainlines.add(line);
		}
	...
	//print all platforms in a train line
	private void printTrainLine(String line){

		Platform currentStation = endOfLine(line);
		HashSet<Integer> seenStationId = new HashSet<>();

		System.out.print(line + ": ");

		while (currentStation != null) {
			System.out.print(currentStation.getStationName() + ", ");
			seenStationId.add(currentStation.getPlatformId());
			boolean isEnd = true;
			for (Integer nextStation: G.adj(currentStation.getPlatformId())){
				if (seenStationId.contains(nextStation)){
					continue;
				}

				// check the next station by this particular line
				Platform candidate = platforms[nextStation];
				if (candidate.getTrainLine().equals(line)){
	...
	public static void main(String[] args) {
		...
		Set<String> lines = mG.findALLTrainLines();
		for (String ln : lines) {
			mG.printTrainLine(ln);
		}
